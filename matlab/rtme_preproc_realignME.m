function output = rtme_preproc_realignME(sub, task, run, reference_echo, defaults)
...
functional_fn = fullfile(preproc_dir, sub, 'func', [sub '_task-' task '_run-' num2str(run) '_echo-' num2str(reference_echo) '_bold.nii']);
scans = {};
...
for e = 1:N_echoes
    ...
    functional_fn = fullfile(preproc_dir, sub, 'func', [sub '_task-' task '_run-' num2str(run) '_echo-' num2str(e) '_bold.nii']);
    rfunctional_fn = fullfile(preproc_dir, sub, 'func', ['r' sub '_task-' task '_run-' num2str(run) '_echo-' num2str(e) '_bold.nii']);
