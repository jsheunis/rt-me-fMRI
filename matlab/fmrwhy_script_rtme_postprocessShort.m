...
for roi = 1:numel(roi_desc)
    col_names = [];
    txt_fn = fullfile(options.deriv_dir, 'fmrwhy-rt', ['sub-' sub], ['sub-' sub '_task-' task '_desc-' roi_desc{roi} '_ROIsignals.txt']);
    tsv_fn = strrep(txt_fn, '_ROIsignals.txt', '_ROIsignals.tsv');
    data = [];
    for sig = 1:numel(sig_desc)
        names = {};
        for n = 1:numel(nfb_sigs) % exclude S0 time series
    ...
    [status, msg, msgID] = movefile(txt_fn, tsv_fn);
end
...
for roi = 1:numel(roi_desc)
    col_names = [];
    txt_fn = fullfile(options.deriv_dir, 'fmrwhy-rt', ['sub-' sub], ['sub-' sub '_task-' task '_desc-' roi_desc{roi} '_ROIpsc.txt']);
    tsv_fn = strrep(txt_fn, '_ROIpsc.txt', '_ROIpsc.tsv');
    txt_cnr_fn = fullfile(options.deriv_dir, 'fmrwhy-rt', ['sub-' sub], ['sub-' sub '_task-' task '_desc-' roi_desc{roi} '_ROIcnr.txt']);
    tsv_cnr_fn = strrep(txt_cnr_fn, '_ROIcnr.txt', '_ROIcnr.tsv');
    txt_tcnr_fn = fullfile(options.deriv_dir, 'fmrwhy-rt', ['sub-' sub], ['sub-' sub '_task-' task '_desc-' roi_desc{roi} '_ROItcnr.txt']);
    tsv_tcnr_fn = strrep(txt_tcnr_fn, '_ROItcnr.txt', '_ROItcnr.tsv');
    ...
    for sig = 1:numel(sig_desc)
        ...
        ts = glmProcTimeSeries{sig}(roi,:);
        glm_psc = (ts./nanmean(ts));
        glm_cnr = nanmean(glm_psc(I_taskPSC)) - nanmean(glm_psc(I_baselinePSC));
        glm_tcnr = glm_cnr/std(glm_psc);

        ts = kalmanProcTimeSeries{sig}(roi,:);
        kalm_psc = (ts./nanmean(ts));
        kalm_cnr = nanmean(kalm_psc(I_taskPSC)) - nanmean(kalm_psc(I_baselinePSC));
        kalm_tcnr = kalm_cnr/std(kalm_psc);

        data = [data glm_psc' kalm_psc'];
        data_cnr = [data_cnr glm_cnr kalm_cnr];
        data_tcnr = [data_tcnr glm_tcnr kalm_tcnr];
    ...
    data_table = array2table(data_cnr,'VariableNames', col_names);
    writetable(data_table, txt_cnr_fn, 'Delimiter','\t');
    [status, msg, msgID] = movefile(txt_cnr_fn, tsv_cnr_fn);

    data_table = array2table(data_tcnr,'VariableNames', col_names);
    writetable(data_table, txt_tcnr_fn, 'Delimiter','\t');
    [status, msg, msgID] = movefile(txt_tcnr_fn, tsv_tcnr_fn);
...
% colorss = {cb(1,:), cb(5,:), cb(4,:), cb(6,:), cb(8,:), cb(10,:), cb(9,:)}
% pp = {};
% roi = 1;
% for i = 1:7
%    pp{i} = plot(1:Ndyn, kalmanProcTimeSeries{i}(roi,:), 'color', colorss{i}, 'LineWidth', 1.5)
% end

% legend('Echo', 'T2{\ast}-combined', 'tSNR-combined', 'TE-combined', 'rtT2{\ast}-combined', 'rtT2{\ast}', 'rtSO')




% for n = 1:N_subplots
%     subplot(N_subplots, 1, n);
%     hold on;
%     [cb] = cbrewer('qual','Set3',12,'pchip');
%     colorss = {cb(1,:), cb(5,:), cb(4,:), cb(6,:), cb(8,:), cb(10,:), cb(9,:)}
%     pp = {};
%     for i = 1:7
%         ts = TS{n}{i}(roi,:);
%         ts_psc = (ts./nanmean(ts));
%         pp{i} = plot(1:Ndyn, ts_psc, 'color', colorss{i}, 'LineWidth', 1.5)

%         if n==2
%             CNR{1,i}  = nanmean(ts(I_taskPSC)) - nanmean(ts(I_baselinePSC));
%             tCNR{1,i} = CNR{1,i}/std(ts);
%             CNR{2,i}  = nanmean(ts_psc(I_taskPSC)) - nanmean(ts_psc(I_baselinePSC));
%             tCNR{2,i} = CNR{2,i}/std(ts_psc);
%         end
%     end
%     legend('Echo', 'T2{\ast}-combined', 'tSNR-combined', 'TE-combined', 'rtT2{\ast}-combined', 'rtT2{\ast}', 'rtSO')
%     title(TS_titles{n});
...
%         tsnr_output_fn = strrep(tSNR_fns{i}, '_tsnr.nii', ['_' masks_oriented.field_names{j} 'tsnr.tsv']) = strrep(tsnr_output_fn, '.tsv', '_temp.txt');
%         data_table = array2table(vals,'VariableNames', {'tsnr'});
%         writetable(data_tab, 'Delimiter','\t');
%         [status, msg, msgID] = movef, tsnr_output_fn);
%     end
...
%             tsnr_output_fn = strrep(tSNR_fns{i}, '_tsnr.nii', ['_' roi_desc_txt{k} masks_oriented.field_names{j} 'tsnr.tsv']); = strrep(tsnr_output_fn, '.tsv', '_temp.txt');
%             data_table = array2table(vals,'VariableNames', {'tsnr'});
%             writetable(data_tab, 'Delimiter','\t');
%             [status, msg, msgID] = movef, tsnr_output_fn);
%         end
