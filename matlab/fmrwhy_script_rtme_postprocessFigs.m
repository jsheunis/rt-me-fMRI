...
% sigsss = [rawTimeSeries{sig}(roi,:)' displRawTimeSeries{sig}(roi,:)' glmProcTimeSeries{sig}(roi,:)' kalmanProcTimeSeries{sig}(roi,:)' scalProcTimeSeries{sig}(roi,:)' norm_percValues{sig}(roi,:)' NFB{sig}(roi,:)' NFB_disp{sig}(roi,:)'];


figure;
roi = 7;
TS = {glmProcTimeSeries, kalmanProcTimeSeries, scalProcTimeSeries, norm_percValues, NFB};
TS_titles = {'Standard out - GLM', 'Standard out - Kalman', 'Standard out - Scaled', 'Standard out - NormPerc', 'Standard out - NFB'};
N_subplots = numel(TS);
for n = 1:N_subplots
    ...
    pp = {};
    for i = 1:7
    pp{i} = plot(1:Ndyn, TS{n}{i}(roi,:), 'color', colorss{i}, 'LineWidth', 1.5)
    end
...
CNR = cell(2, 7);
tCNR = cell(2, 7);
TS_titles = {'PSC out - GLM', 'PSC out - Kalman', 'PSC out - Scaled', 'PSC out - NormPerc', 'PSC out - NFB'};
for n = 1:N_subplots
    subplot(N_subplots, 1, n);
    hold on;
    [cb] = cbrewer('qual','Set3',12,'pchip');
...
figure;
TS_titles = {'rtPSC out - GLM', 'rtPSC out - Kalman', 'rtPSC out - Scaled', 'rtPSC out - NormPerc', 'rtPSC out - NFB'};
for n = 1:N_subplots
    subplot(N_subplots, 1, n);
    hold on;
    [cb] = cbrewer('qual','Set3',12,'pchip');
    ...
    for i = 1:7
        ...
        ts_psc = nan(size(ts));
        for j = 1:numel(ts)
            i_bas = I_baselinePSC(I_baselinePSC<j);
            if task_designPSC(j)
                ts_psc(j) = (ts(j)./nanmean(ts(i_bas)));
            else
            end
        end
        pp{i} = plot(1:Ndyn, ts_psc, 'color', colorss{i}, 'LineWidth', 1.5)
