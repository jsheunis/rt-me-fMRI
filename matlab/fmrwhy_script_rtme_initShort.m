...
% Templates
% functional0_fn = fullfile(options.preproc_dir, ['sub-' sub], 'func', ['sub-' sub '_task-' options.template_task '_run-' options.template_run '_space-individual_bold.nii']);
functional0_fn = options.template_fn;
...
for e = 1:numel(echoes)
    if e ==1
        [filename, filepath] = fmrwhy_bids_constructFilename('func', 'sub', sub, 'task', task, 'echo', echoes{e}, 'desc', 'rapreproc', 'ext', '_bold.nii');
        boldts_fn{e} = fullfile(options.preproc_dir, filepath, filename);
    else
        [filename, filepath] = fmrwhy_bids_constructFilename('func', 'sub', sub, 'task', task, 'desc', echoes{e}, 'ext', '_bold.nii');
        boldts_fn{e} = fullfile(options.me_dir, filepath, filename);
    end
end
...
masks = fmrwhy_util_loadMasksSPM(options.bids_dir, sub);
N_tissuemasks = 4;
% Get task-based regions of interest (resliced and in subject functional space)
N_taskROIs = 7; %numel(options.roi.(task).rroi_fn);
...
FWE_dir_stats = fullfile(options.stats_dir, ['sub-' sub], ['task-' task '_echo-' echo]);
noFWE_dir_stats = fullfile(options.stats_dir, ['sub-' sub], ['task-' task '_echo-' echo '_noFWEp001e20']);

% if strcmp(task, 'motor')
%     roi_anat_fn = fullfile(options.preproc_dir, ['sub-' sub], 'anat', ['sub-' sub '_space-individual_desc-rleftMotor_roi.nii']);
...
% else
%     roi_anat_fn = fullfile(options.preproc_dir, ['sub-' sub], 'anat', ['sub-' sub '_space-individual_desc-rbilateralAmygdala_roi.nii']);
...
% Anatomical ROIs
r = 1;
ROI_fns{r} = fullfile(options.preproc_dir, ['sub-' sub], 'anat', ['sub-' sub '_space-individual_desc-rleftMotor_roi.nii']);
...
ROI_names{r} = 'rleftMotor';

r = 2;
ROI_fns{r} = fullfile(options.preproc_dir, ['sub-' sub], 'anat', ['sub-' sub '_space-individual_desc-rbilateralAmygdala_roi.nii']);
ROI_img{r} = spm_read_vols(spm_vol(ROI_fns{r}));
ROI_img{r}(isnan(ROI_img{r})) = 0;
ROI_img{r} = (ROI_img{r} > 0.1);
I_roi{r} = find(ROI_img{r}(:)); % find(ROI_img{r}(:) & GM_img_bin(:)); TODO, investigate which masking to use here, if any
ROI_names{r} = 'rbilateralAmygdala';

r = 3;
ROI_fns{r} = fullfile(options.preproc_dir, ['sub-' sub], 'anat', ['sub-' sub '_space-individual_desc-rfusiformGyrus_roi.nii']);
ROI_img{r} = spm_read_vols(spm_vol(ROI_fns{r}));
ROI_img{r}(isnan(ROI_img{r})) = 0;
ROI_img{r} = (ROI_img{r} > 0.1);
I_roi{r} = find(ROI_img{r}(:)); % find(ROI_img{r}(:) & GM_img_bin(:)); TODO, investigate which masking to use here, if any
ROI_names{r} = 'rfusiformGyrus';

% Binary clusters: FWE, noFWE, AND, OR
if strcmp(task, 'emotionProcessing')
    k = 3;
else
    k = 1;
end
r = 4;
ROI_fns{r} = fullfile(FWE_dir_stats, ['spmT_' sprintf('%04d', k) '_binary_clusters.nii']);
ROI_img{r} = spm_read_vols(spm_vol(ROI_fns{r}));
ROI_img{r}(isnan(ROI_img{r})) = 0;
ROI_img{r} = (ROI_img{r} > 0.1);
I_roi{r} = find(ROI_img{r}(:)); % find(ROI_img{r}(:) & GM_img_bin(:)); TODO, investigate which masking to use here, if any
...
r = 5;
ROI_fns{r} = fullfile(noFWE_dir_stats, ['spmT_' sprintf('%04d', k) '_binary_clusters.nii']);
ROI_img{r} = spm_read_vols(spm_vol(ROI_fns{r}));
ROI_img{r}(isnan(ROI_img{r})) = 0;
ROI_img{r} = (ROI_img{r} > 0.1);
I_roi{r} = find(ROI_img{r}(:)); % find(ROI_img{r}(:) & GM_img_bin(:)); TODO, investigate which masking to use here, if any
...
r = 6;
ROI_fns{r} = fullfile(options.stats_dir, ['sub-' sub], ['sub-' sub '_task-' task '_desc-allEchoTaskClustersAND.nii']);
ROI_img{r} = spm_read_vols(spm_vol(ROI_fns{r}));
ROI_img{r}(isnan(ROI_img{r})) = 0;
ROI_img{r} = (ROI_img{r} > 0.1);
I_roi{r} = find(ROI_img{r}(:)); % find(ROI_img{r}(:) & GM_img_bin(:)); TODO, investigate which masking to use here, if any
...
r = 7;
ROI_fns{r} = fullfile(options.stats_dir, ['sub-' sub], ['sub-' sub '_task-' task '_desc-allEchoTaskClustersOR.nii']);
ROI_img{r} = spm_read_vols(spm_vol(ROI_fns{r}));
ROI_img{r}(isnan(ROI_img{r})) = 0;
ROI_img{r} = (ROI_img{r} > 0.1);
I_roi{r} = find(ROI_img{r}(:)); % find(ROI_img{r}(:) & GM_img_bin(:)); TODO, investigate which masking to use here, if any
...
initLim = {};
N_roi = N_ROIs;
for sig = 1:numel(echoes)
    tic;
    disp(['Loading sig = ' echoes{sig}]);
    % currentVol{sig} = spm_vol(boldts_fn{sig});
    % F_dyn_img{sig} = spm_read_vols(currentVol{sig}); % Read image volume with SPM. This is the raw/unprocessed image
    % F{sig}(:,j) = F_dyn_img{sig}(:);

    % F{sig} = nan(Nx*Ny*Nz,Ndyn);
    % % F_dyn_denoised{e} = nan(Nx*Ny*Nz,Ndyn);
    % % rF{e}  = nan(Nx*Ny*Nz,Ndyn);
    % reshape(me_params.T2star_3D, Nx*Ny*Nz, 1);

    signals_raw_3D{sig} = spm_read_vols(spm_vol(boldts_fn{sig}));
    signals_raw{sig} = reshape(signals_raw_3D{sig}, Nx*Ny*Nz, Ndyn);
